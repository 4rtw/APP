...
function getEtudiant(req, res){
    ...
    Etudiants.findOne(condition,(err,etudiant)=>{
        ...
        if(!etudiant){
            console.warn(`Impossible de trouver l'etudiant  -> [ID = ${condition.matricule}]`)

            return res.status(404).json(apiResponse({
                data: [],
                status: 0,
                errors: [MSG.HTTP_404],
                message: `Le etudiant [ID = ${condifion.matricule}] n'existe pas`
            }))
        }
...
function postEtudiant(req, res){
    ...
    etudiant.contacts = req.body.contacts

    console.log("POST etudiant reçu :" + etudiant)

    const manyErrors = validate(etudiant, ['id', 'matricule','nom', 'prenom', 'contacts'])
...
function updateEtudiant(req, res) {
    ...
    Etudiants.findOneAndUpdate(condition, req.body, opts, (err, etudiant) => {
        ...
        if(!etudiant){
            console.warn(`Impossible de trouver l'étudiant  -> [ID = ${condifion.matricule}]`)

            return res.status(404).json(apiResponse({
                data: [],
                status: 0,
                errors: [MSG.HTTP_404],
                message: `L'étudiant [ID = ${condifion.matricule}] n'existe pas`
            }))
        }
...
function deleteEtudiant(req, res) {
    ...
    Etudiants.findOneAndRemove(condition, (err, etudiant) => {
        ...
        if(!etudiant){
            console.warn(`Impossible de trouver l'etudiant  -> [ID = ${condifion.matricule}]`)

            return res.status(404).json(apiResponse({
                data: [],
                status: 0,
                errors: [MSG.HTTP_404],
                message: `L'etudiant [ID = ${condifion.matricule}] n'existe pas`
            }))
        }
...
module.exports = {getEtudiants,getEtudiant,searchEtudiants,postEtudiant,updateEtudiant,deleteEtudiant}
